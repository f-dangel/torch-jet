  ...
  \begin{split}
      ...
      &\overset{\text{(\ref{eq:faa-di-bruno})}}{\to}
        ...
        &\overset{\text{sum}}{\to} \sum_{r=1}^R \vg_{K,r} 
          \overset{\{\vx_{1,r} = \vv_r, \vx_{2,r} =  \dots = \vx_{K, r} = \vzero \}}{=} 
          \sum_{r=1}^R \left<
          \partial^K \vf(\vx_0), 
          \otimes_{k=1}^K \vv_r
          \right>
...
Here, we will show details about the propagation schemes of the standard Taylor mode and the collapsed Taylor mode for the computation of the Laplacian of $\vf$. We consider the decomposition $\vf = \vg \circ \vh$.

\paragraph{Standard Taylor Mode AD}
Using standard Taylor mode (see \cref{eq:sum-taylor-mode-naive}) to compute the Laplacian we obtain
{

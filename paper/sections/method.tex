...
\todo{T: Working on it.}
To extend our method, we start with a generalized version of \cref{eq:sum-k-directional}. We assume there are $P$ different directions: \todo{T: I'm sorry, mir fällt gerade nichts besseres ein}
\begin{align}\label{eq:sums-k-directional}
  \sum_{n_1=1}^{N_1} \dots \sum_{n_P=1}^{N_P}\left< 
  \partial^{K} f(\vx_0),
  \otimes_{k=1}^{p_{n_1}}\vv_{n_1} \otimes \dots \otimes_{k=1}^{p_{n_P}}\vv_{n_P}
  \right>,
\end{align}
where $\vp = (p_1, \dots, p_P)$ are defined by the differential operator such that $\sum_i p_i = K$. What seems complicated is indeed the framework of to handle operators like the Biharmonic Operator (see \cref{sec:example2}). In general, there are summands where some of the direction vectors $\vv_{n_k}$ do not coincide. Those summands are not directly representable by $K$-Jets and thus, not computable via collapsed Taylor-mode. However, instead of relying on nested derivative calls we are going to apply the technique of Griewank et al. to reconstruct a mixed-partial of degree $K$ from a family of $K$-Jets. In the next paragraph we transfer the notation of \cref{eq:sums-k-directional} into the setting of the work of Griewank et al.
Let $P_{n_1 \dots n_K}$ be the number of distinct $\vv_{n_k}$ of the corresponding summand and $\vp = (p_1, \dots, p_P)$ the number of the occurrences of the vectors. Stacking the distinct vectors into a single matrix $\mV_{n_1 \dots n_K} \in \mathbb{R}^{D \times P_{n_1 \dots n_K}}$ allows the following reformulation of \cref{eq:sums-k-directional}:
\begin{equation}
    \left. 
    \frac{d^K}{d\vz_{n_1 \dots n_K}^\vp }
    \vf
    \left(
    \vx_0 + \mV_{n_1 \dots n_K} \vz_{n_1 \dots n_K}
    \right)
    \right|_{\vz_{n_1 \dots n_K} = \vzero}
\end{equation}
The equation above is formulated in multi-index notation, which is further specified in \cref{sec:appendix_ttc}. Note, apart from the alignment of the order of $\vv_{n_k}$ between $\vp$ and $\mV_{n_1 \dots n_K}$ the order is not relevant because of the symmetry of the tensor product.
Griewank et al. found that this equation equals a certain sum of $K$-Jets
\begin{equation}
\label{eq:ttc_general}
    \sum_{\underset{\vq \in \mathbb{N}^{P{n_1 \dots n_K}}}{|\vq| = |\vp|}}
    \gamma_{\vp \vq}
    \frac{1}{\vp!}
    \left<
    \partial^{K}\vf,
...
\cref{sec:appendix_ttc} provides supplementary details to the multi-index notation of the formula. From \cref{eq:ttc_general} we conclude
\begin{align} \label{eq:ttc_general_operator}
  ...
  \right>
  = 
  \sum_{n_1=1}^{N_1} \dots \sum_{n_K=1}^{N_K} 
  \sum_{\underset{\vq \in \mathbb{N}^{P{n_1 \dots n_K}}}{|\vq| = |\vp|}}
    \gamma_{\vp \vq}
    \frac{1}{\vp!}
    \left<
    ...
    \right>.
\end{align}
Therefore, a general differential operator with arbitrary mixed-partial derivatives is representable through a family of $K$-Jets. Thus, every operator in the form of $\cref{eq:ttc_general}$ can be computed via our collapsed Taylor-mode by propagating the different $K$-Jets with directions $\mV_{n_1 \dots n_K}\vq$ instead of depending on expensive nested derivative calls. 

As the inventors of the formula already recognized, the coefficients $\gamma_{\vp\vq}$ only depend on the problem structure \cref{eq:ttc_general} and are independent of the function $\vf$. As we will show, this allows us to exploit symmetry that is often inherent to differential operators.
...
\label{eq:biharm}
    ...
    \right>,
\end{align}
and therefore a special case of \cref{eq:sums-k-directional} if we select $K = 4, P = 2, N_1 = N_2 = D$, $\vv_{n_1} = \ve_{n_1}$ and $\vv_{n_2} = \ve_{n_2}$.

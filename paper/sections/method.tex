...
\begin{align}\label{eq:laplacian}
  ...
  \right>
\end{align}
Assume that $f$ is again decomposed into $g \circ \vh$, where $\vh$ is vector valued, as is the case if $\vh$ is a hidden layer of a neural network. Selecting $K = 2$, $N=D$ and $\vv_n = \ve_n$ lifts \cref{eq:laplacian} into the setting of \cref{eq:sum-k-jet}.
...
\end{align}
This expression represents the diffusion term in a Kolmogorov-type partial differential equation \cite{hu2023hutchinson}. It is often equivalently written as $\Tr (\langle \msigma \msigma^\top, \partial^2 \vf(\vx_0))\rangle$, where $\msigma$ may be a function $\msigma:\mathbb R^{D}\to \mathbb R^{D\times D}$. Since $\boldsymbol{C}$ is positive semi-definite it can be expressed as $\mC = \sum_{n=1}^{\rank(\mC)} \vc_n \vc_n^\top$ for suitable $\vc_n \in \sR^D$. This matches the setting of \cref{eq:sum-k-jet} with $K = 2, N = \rank(\mC)$ and $\vv_n = \vc_n$, and can be computed using \cref{eq:sum-taylor-mode-efficient}. This requires the propagation of $1 + \rank(\mC) + 1$ vectors instead of $1 + 2\rank(\mC)$.
...
\todo{cite papers that consider this. M: I am not so sure where this opeartor appears naturally} The higher-order Laplacian, an often theoretically considered example, is also easily expressible in our framework:

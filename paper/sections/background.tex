...
Naively we get $(2 + D) + D * (2 + D) + D * (2 + D)$, but the second-order components can again be collapsed into a single vector, so we obtain \todo{Are you sure that we can contract $D(2 + D)$ into $1$? Shouldn't it be $2 + D$?}$(2 + D) + D * (2 + D) + 1 = D^2 + 3D + 3$ vectors that need to be pushed forward for the biharmonic.
\textbf{We should expect Tim's $D^2 + 2D + 3$ from below though, so there is still a slight mismatch we need to understand.}
\textbf{Tim: In general: I'm pretty sure you can not expect the same results as in the multitvariate-propatation case. In the multivariate case you can leverage the linearity in terms where different directions are propagated. In univariate you can not. This can be seen from the different chain-rules.}
\\
\textbf{
Regarding the propagation scheme above, we can only leverage the trick for the second-order terms, but not for third-order, because the scheme can only see up to the second derivative. But in the multivariate case we can leverage the trick to safe $D$ third derivatives and $D^2-1$ forth. Instead we can not safe the third derivatives, but $D$ fourth derivatives, since we can apply the laplace trick here.}


\paragraph{Leveraging TTC formula} \todo{Not done}\todo{Need better notation}
...
\label{eq:ttc_general}
    \frac{\partial^{|\vk|}}{\partial \vz^\vk} \vf(\vx + \boldsymbol{S} \vz)\Big|_{\vz = 0} 
    = 
    \sum_{\underset{\vl \in M^p}{|\vl| = |\vk|}}
    \gamma_{\vk \vl}
    \frac{1}{\vk!}
    \partial^{|\vk|}\vf \left[
    \underbrace{
    \sum_{i=1}^p l_i \vs_i, \dots, \sum_{i=1}^p l_i \vs_i
    }_k
    \right].
\end{equation}
\cref{sec:appendix_ttc} provides supplementary details to the formula.
As the authors of the formula already noted, the coefficients $\gamma$ only depend on the problem structure and are independent of the function $\vf$. As we will show, this allows to exploit symmetry that is often inherent to differential operators.

To apply \cref{eq:ttc_general} to the Biharmonic Operator \cref{eq:biharm}, parameters $\vk, p$, auxiliary variables $\vz$ and matrices $\boldsymbol{S}_{ij}$ has to be selected for all mixed-partial derivatives. For the case where all four derivative directions coincides, i.e.,$i = j$, the parameters are given by $\vk = (4)$, $p = 1$, the auxiliary variable $\vz = (z_1)$ and $\boldsymbol{S}_{ii} = \ve_i$. If $i \neq j$ we have $\vk = (2, 2)$, $p = 2$, $\vz = (z_1, z_2)$ and $\boldsymbol{S}_{ij} = (\ve_i \; \ve_j)$. 

Having defined the parameters, we utilize \cref{eq:ttc_general} to map the Biharmonic Operator \cref{eq:biharm} of a function $\vf$ to a collection of fourth-order jets: 
\begin{align}
    \Delta^2 \vf(\vx)
    &=
    \sum_{i=1}^D
    ...
    \frac{1}{4!}
    \partial^{4}\vf \left[
    4 \ve_i, 4 \ve_i, 4 \ve_i, 4 \ve_i 
    \right]
    \\
    &+
    \sum_{i=1}^D \sum_{\underset{j \neq i}{j=1}}^D
    \sum_{\underset{\vl \in M^2}{|\vl| = 4}}
    \gamma_{(2, 2) \vl}
    \frac{1}{(2, 2)!}
    \partial^{4} \vf \left[
    \boldsymbol{S}_{ij} \vl, \boldsymbol{S}_{ij} \vl, \boldsymbol{S}_{ij} \vl, \boldsymbol{S}_{ij} \vl
    \right].
\end{align}
As mentioned before, the coefficients $\gamma_{\vk \vl}$ capture the symmetric structure of the differential operator. A close look into the definition \cref{eq:ttc_coeff} shows the equality of $\vl = (4,0)$ with $\vl=(0, 4)$ and $\vl = (3, 1)$ with $\vl = (3, 1)$. Exploiting those symmetries the formula boils down to 
\begin{align}
   \Delta^2 \vf(\vx)
   &=
    \left(
   ...
   \sum_{i=1}^D
    ...
    \sum_{\underset{j \neq i}{j=1}}^D
    \partial^4 \vf[
    3 \ve_i+ \ve_j, 3 \ve_i+ \ve_j, 3 \ve_i+ \ve_j, 3 \ve_i+ \ve_j
    ]
    \\
    &+ 
    \gamma_{(2, 2)(2, 2)}
    ...
    \sum_{\underset{j \neq i}{j=1}}^D
    \partial^4 \vf [
    2 \ve_i+ 2 \ve_j, 2 \ve_i+ 2 \ve_j, 2 \ve_i+ 2 \ve_j, 2 \ve_i+ 2 \ve_j
    ].
\end{align}

\todo{Then @Felix can implement and benchmark this approach. 
This is probably better than the current approach which uses 6-jets.
And it uses 4-jets, which are also used by the randomized variant.
We should see a stronger performance impact when collapsing the highest-order coefficients, compared to the 6-jet approach.}

\Cref{tab:ttc_biharm_coeffs} lists all required coefficients.
...
\begin{table}[]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \midrule
        $\frac{3}{32}$ & $\frac{13}{192}$ & $\frac{5}{8}$ & -$\frac{1}{3}$ \\
        \bottomrule
    \end{tabular}
...
Leveraging the symmetry of the direction $2 \ve_i + 2 \ve_j$, the last term can be further simplified. This simplification leads to the following efficient propagation scheme for \cref{eq:biharm}:
\begin{align}
   \Delta^2 \vf(\vx)
...
The \Crefrange{eq:ttc_biharm_full1}{eq:ttc_biharm_full3} are computed through $D + D(D-1) + \frac{D(D-1)}{2}$ fourth-order jets. All these fourth-order jets expose the same structure as need for our proposed propagation trick. Thus, the summation can be pulled inside the highest coefficients in the propagation scheme, like for the Laplacian. From this we conclude that this propagation scheme has the effort of propagating $1 + 3D + 1 + 1 + 3 \cdot D(D-1) + 1 + 1 + 3 \frac{D(D-1)}{2} + 1 = 9\frac{D^2}{2} - 3\frac{D}{2} + 6$ vectors. It should be noted, that all jets can be computed in parallel, which provides further benefits for the run-time complexity.

...
\begin{columns}
  ...
  \block{Background: What is Taylor Mode Autodiff?}{
    ...
    \begin{center}
        {\tikzset{%
            every matrix/.append style={column sep=1ex, row sep=1.25ex},%
            every node/.append style={scale=0.375},%
            every path/.append style={line width=1.25pt},%
            Stealth/.append style={scale=1},%
          }
          \begin{tikzpicture}[scale=0.5, every node/.style={scale=0.5, transform shape}]
            \node [inner xsep=40pt, inner ysep=15pt, draw=tab-orange, rounded corners=7.5pt, line width=5pt] (graph-before) {\input{../paper/figures/sin_2jet_5.tex}};
            \coordinate (separator) at ($(graph-before.north east)!0.46!(graph-before.south east)$);
            \draw[tab-orange, fill=tab-orange, line width=2.5pt, rounded corners=7.5pt] (graph-before.south west) rectangle (separator);
            \node [inner xsep=40pt, inner ysep=15pt, draw=tab-orange, rounded corners=7.5pt, line width=5pt] at (graph-before) {\input{../paper/figures/sin_2jet_5.tex}};
            \node [anchor=north east, align=center,font=\bfseries\LARGE] at (separator) {Propagate then sum};
          \end{tikzpicture}
        }
    \end{center}
  }
  ...
  \block{Main Idea: Use Linearity to Collapse the Propagation}{
    ...
    \begin{center}
        {\tikzset{%
          ...
          \begin{tikzpicture}[scale=0.5, every node/.style={scale=0.5, transform shape}]
            \node[inner xsep=40pt, inner ysep=10pt, draw=tab-green, rounded corners=7.5pt, line width=5pt] (graph-after) {\input{../paper/figures/sin_2jet_8.tex}};

            \coordinate (separator) at ($(graph-after.north east)!0.46!(graph-after.south east)$);
            \draw [tab-green, line width=2.5pt, rounded corners=7.5pt, fill=tab-green] (graph-after.south west) rectangle (separator);
            % Re-draw
            \node[inner xsep=40pt, inner ysep=10pt, draw=tab-green, rounded corners=7.5pt, line width=5pt] at (graph-after) {\input{../paper/figures/sin_2jet_8.tex}};

            \node [anchor=north east, align=center,font=\bfseries\LARGE] at (separator) {Sum then propagate};
          \end{tikzpicture}
        }
    \end{center}
  }

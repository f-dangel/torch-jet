...
A different technique to reconstruct mixed-partials from jets was already proposed by Griewank et al in 1999 in \cite{griewank_evaluating_1999}. The authors summarized their method in the following formula
\begin{equation}
...
For the Biharmonic Operator, the parameters differ between the case $i=j$ and $i \neq j$. When $i = j$ it is $k = (4)$, $p = 1$, $S_{ii} = e_i$. For $i \neq j$, $k = (2, 2)$, $p = 2$ and for every term, $S_{ij} = (e_i \; e_j)$. Thus, 
\begin{align}
        \sum_{i=0}^D \sum_{j=0}^D
        \frac{\partial^|k|}{\partial z^k} f(x + Sz)\Big|_{z = 0} &= 
        \sum_{i=0}^D 
        \frac{\partial^4}{\partial z_1^4} f(x + z_1 e_i)\Big|_{z_1 = 0} 
        +
        \sum_{i=0}^D 
        \sum_{\underset{j \neq i}{j=0}}^D 
        \frac{\partial^4}{\partial z_1^2 \partial z_2^2} f(x + z_1 e_i + z_2 e_j)\Big|_{z = 0}.
\end{align}
A close look into the definition of the coefficients $\gamma_{kl}$ shows that there are independent of $i, j$ and they coincide if $l = (4,0)$ or $l=(0, 4)$ as well as if $l = (3, 1)$ or $l = (3,1)$. Leveraging these facts we end up with the following formula
\begin{align}
   \sum_{i=1}^D \sum_{j=1}^D \frac{\partial^4}{\partial x_i^2 \partial x_j^2} f(x)
   &=
    \left(
   \gamma_{44} + 2 (D - 1) \gamma_{(2,2)(4, 0)}
   \right)
   \sum_{i=0}^D
    f^4(x; 4 e_i)
    \\
    ...
    f^{4}(x; 2 e_i+ 2 e_j).
\end{align}
@Tim: Can you write down the values for the $\gamma$s?
Then @Felix can implement and benchmark this approach. 

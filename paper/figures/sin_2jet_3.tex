{
  ...
  \begin{tikzpicture}
    ...
    row sep=4ex,%
      ...
      \\
      \& \node[state, replicate] (df-r) {df\_r = replicate(df)};
      \\
      \node[state, leaf] (x1-d) {x1\_r};
      \&
      \& \node[state] (temp1-d) {temp1\_r = einsum(``...,...,...\\->...'', d2f\_r, x1\_r, x1\_r)};
      \& \node[state, output] (f1-d) {f1\_r =
        einsum(``...,...\\->...'', df\_r, x1\_r)};
      \\
      \node[state, leaf] (x2-d) {x2\_r};
      \& \node[state] (temp2-d) {temp2\_r = einsum(``...,...\\->...'', df\_r, x2\_r)};
    ...
    (df-d) edge [arrow] (df-r)
    (x0-d) edge [arrow] (d2f-d)
    (df-r) edge [arrow, out=-8, in=169] (f1-d)
    (x1-d) edge [arrow, out=10, in=167] (f1-d)
    (df-r) edge [arrow] (temp2-d)

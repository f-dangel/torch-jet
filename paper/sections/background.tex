...
As the authors of the formula already noted, the coefficients $\gamma$ only depend on the problem structure and are independent of the function $\vf$. As we will show, this allows to exploit symmetry that is inherent to differential operators.


\cref{eq:ttc_general} can be leveraged to map the Biharmonic Operator to a collection of fourth-order jets:

\begin{equation}
     \Delta^2 \vf (\vx)
     =  
     \sum_{i=1}^D \sum_{j=1}^D
     \frac{\partial^{4}}{\partial z_1^2 \partial z_2^2} \vf(\vx + \boldsymbol{S}_{ij} \vz)\Big|_{\vz = 0} 
\end{equation}
, which are then handled in our proposed propagation framework. 

For the Biharmonic Operator, the parameters differ between cases $i=j$ and $i \neq j$. When $i = j$ it is $\vk = (4)$, $p = 1$, $\boldsymbol{S}_{ii} = \ve_i$. For $i \neq j$, $\vk = (2, 2)$, $p = 2$ and for every term, $\boldsymbol{S}_{ij} = (\ve_i \; \ve_j)$. Thus, 
\begin{align}
        \sum_{i=1}^D \sum_{j=1}^D
        \frac{\partial^4}{\partial \vz_1^2 \partial \vz_2^2} 
        \vf(\vx + \boldsymbol{S}\vz)
        \Big|_{\vz = \boldsymbol{0}}
        &=
        \sum_{i=1}^D 
        \frac{\partial^4}{\partial \vz_1^4} 
        \vf(\vx + \vz_1 \ve_i)
        \Big|_{\vz_1 = 0} \\
        &+
        \sum_{i=1}^D 
        \sum_{\underset{j \neq i}{j=1}}^D 
        \frac{\partial^4}{\partial \vz_1^2 \partial \vz_2^2} 
        \vf(\vx + \vz_1 \ve_i + \vz_2 \ve_j)
        \Big|_{\vz = \boldsymbol{0}}.
\end{align}
A close look into the definition of the coefficients $\gamma_{\vk \vl}$ shows that there are independent of $i, j$ and they coincide if $\vl = (4,0)$ or $\vl=(0, 4)$ as well as if $\vl = (3, 1)$ or $\vl = (3,1)$. Leveraging these facts we end up with the following formula
\begin{align}
\label{eq:ttc_biharm}
   \sum_{i=1}^D \sum_{j=1}^D 
   \frac{\partial^4}{\partial \vx_i^2 \partial \vx_j^2} 
   \vf(x)
   &=
    \left(
...
The coefficients are given in \cref{tab:ttc_biharm_coeffs}.
...
\begin{table}[]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \bottomrule
    \end{tabular}
    \caption{Coefficients to compute the Biharmonic Operator by \cref{eq:ttc_biharm}.}
    \label{tab:ttc_biharm_coeffs}
\end{table}

In this case, we have to propagate $D + 2 D(D-1)$ fourth-order jets. All these fourth-order jets rely on the same derivative evaluation point $\vx_0$ and the summation can be pulled inside the highest coefficients in the propagation scheme, like for the Laplacian. From this we conclude that this propagation scheme has the effort of propagating $1 + 3D + 1 + 2 \left(1 + 3 \cdot D(D-1) + 1 \right) = 6D^2 - 3D + 6$ vectors. Comparing the memory complexity with the memory complexity of the previous result, it can be seen that there is an overhead. However, all jets can be computed in parallel, which provides benefits for the run-time complexity.

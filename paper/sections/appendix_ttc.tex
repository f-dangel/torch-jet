...
The right side of the formula sums over all multi-indices $\vq = (q_1, \dots q_P) \in \mathbb{N}^p$ such that $|\vq| := \sum_i q_i = K$. If $p=2$ and $|\vq| = 4$ this index-set consists of $\left\{ (4,0), (0, 4), (3, 1), (1, 3),  (2, 2)\right\}$. 

The coefficient $\gamma_{\vp \vq}$ is given as
\begin{equation}
\label{eq:ttc_coeff}
    \gamma_{\vp \vq} = \sum_{0 < \vm \leq \vp} (-1)^{|\vp - \vm|} 
    \left(
    \begin{matrix}
        \vp \\
        \vm
    ...
    \right)^{|\vp|}.
\end{equation}
The summation ranges over the set $\left\{ \vm = (m_1, \dots, m_p) \in \mathbb{N}^p \mid m_1 \leq p_1, \dots, m_p \leq p_P, |m| > 0 \right\}$. Furthermore, we utilize the generalized binomial coefficient
\begin{equation}
    \left(
       \begin{matrix}
        a \\
...
\begin{equation} \label{eq:ttc_for_biharm}
    ...
    \end{aligned}
\end{equation}
Afterwards, we exploit the symmetry of the coefficients $\gamma_{(2, 2)(4, 0)} = \gamma_{(2, 2)(0, 4)}$ and $\gamma_{(2, 2)(3, 1)} = \gamma_{(2, 2)(1, 3)}$
...
\begin{equation} \label{eq:ttc_for_biharm3}
    ...
    \end{aligned}
\end{equation}
Exploiting furter symmetries, one obtains
...
A different approach was proposed in \cite{shi2024stochastic}, which relies on the hand-selection of certain $6$-Jets to extract the searched derivatives. The degree and directions for the Jets are obtained by considering the Faà di Bruno formula for the 6-th coefficient $\vf_6$. Selecting coefficients of the input $6$-Jet to $\vx_1 = \ve_{n_1}, \vx_2 = \ve_{n_2}$ and  $\vx_3 = \vx_4 = \vx_5 = \vx_6 = \vzero$ leads us to

We now describe how to collapse the Taylor mode AD computation of popular linear PDE operators, their stochastic approximations proposed in \cite{shi2024stochastic}, and provide a general recipe for computing and collapsing general linear differential operators by interpolation, using earlier work from \citet{griewank_evaluating_1999}.
At its core, our procedure 
use the linearity of the highest Taylor coefficient's propagation rule.
...
This is the only term that uses the highest input jet's highest coefficient $\vh_{K,r}$, and this dependency is \emph{linear}.
Separating it in the highest coefficient's forward propagation, we get ($\nu(\tilde{\sigma}) = 1$)
\begin{subequations}\label{eq:faa-di-bruno-expanded}
\begin{align}
  %\textstyle % Comment out this line if we have enough space
  \sum_r\vf_{K, r}
  &=
...
Hence, to compute the sum $\sum_r \vg_{K, r}$ we can directly propagate the sum $\sum_r \vh_{K,r}$, collapsing them over all directions.
...
These savings of $R-1$ coefficients are significant improvements over standard Taylor mode, as we discuss below.
In the following, we show how to collapse the standard Taylor mode computation of various PDE operators.

\subsection{Second-order Operators}\label{sec:second-order-operators}
...
Note how we can seamlessly also collapse the stochastic approximation over the randomly sampled directions.
...
Fortunately, \citet{griewank_evaluating_1999} derived an approach to reconstruct such mixed-direction terms by linearly combining a \emph{family} of $K$-jets that is determined by all vectors $\vj \in \sN^I$ whose entries sum to $K$, see \cref{fig:ttc_biharm_coeffs} for an illustration for the biharmonic (5 directions).
...
Exploiting those symmetries reduces the number of interpolation terms from 5 to 3 (\cref{eq:ttc_for_biharm_2}), corresponding to $3D^2$ $4$-jets.
...
The scheme we propose based on \citet{griewank2008evaluating}'s interpolation result allows to calculate \emph{general} linear differential operators beyond Laplacians, and is amenable to collapsing.
Admittedly, \cref{eq:ttc-general} seems daunting at first glance.
However, it (i) offers a one-fits-all recipe to construct computational schemes, and (ii) does not use jets of order $K'>K$ to compute $K$-th order derivatives.

...
In the following, we explain how to transform a differential operator containing arbitrary mixed-partial derivatives into a combination of Jets using the result of Griewank et al. \cite{griewank_evaluating_1999} that is well-known in the AD community.

The considerations start with the generalized version of \cref{eq:sum-k-directional}:
\begin{align}\label{eq:sums-k-directional}
  ...
  \right>.
\end{align}
This operator assumes the independent summation over the direction vectors $\vv_{n_k}$. For distinct $\vv_{n_k}$, the summands are not representable as a single coefficient of a $K$-Jet and thus, not computable via collapsed Taylor-mode. \todo{T: until here}
\Cref{eq:sums-k-directional} includes operators like the Biharmonic Operator for $K = 4, ....$

if $\vk_n = (0) \in \mathbb{N}^D$. The technique of Griewank et al. to reconstruct a mixed-partials of degree $K$ from $K$-Jets has been summarized in the formula
\begin{equation}

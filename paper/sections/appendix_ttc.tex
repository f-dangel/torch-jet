...
The right side of the formula sums over all $\vj \in \sN^I$ such that $\lVert \vj \rVert_1 := \sum_i [\vj]_i = K$.
For example, if $I=2$ and $\lVert \vj \rVert_1 = 4$, this index-set consists of $\left\{ (4,0), (0, 4), (3, 1), (1, 3), (2, 2)\right\}$.
...
Let's compute the coefficient $\gamma_{(2, 2),(3, 1)}$, used by the biharmonic operator:
\begin{align*}
...
\begin{equation} \label{eq:ttc_for_biharm}
  ...
  \end{aligned}
\end{equation}
Now, exploit the symmetry of the coefficients $\gamma_{(2, 2), (4, 0)} = \gamma_{(2, 2), (0, 4)}$ and $\gamma_{(2, 2), (3, 1)} = \gamma_{(2, 2), (1, 3)}$:
...
\item Apply the interpolation equation (\cref{eq:ttc_general}):
  ...
  However, the full potential of requires some further steps that leverage the structure.

...
def jet_convolution(
    ...
    *conv_args: object,
    ...
    Convolution is *bilinear* in ``(input, weight)`` and affine in ``bias`` --
    the convolutional analogue of :func:`jet_mm` / :func:`jet_addmm`, with
    ``input`` and ``weight`` playing the roles of ``mat1`` and ``mat2``. All
    three operand patterns are handled: only ``input`` is a jet, only
    ``weight`` is a jet (e.g. differentiating w.r.t. the kernel), or both are
    (via the Leibniz product rule :func:`_leibniz`). The ``bias`` lands on the
    primal only and must be a constant ``Tensor`` or ``None`` -- a
    Taylor-expanded bias is not supported. Grouped/depthwise convolution needs
    no special handling.
...
def _gather_at_indices(c: Tensor, indices: Tensor) -> Tensor:
    ...
    ``indices`` (shape ``(N, C, oH, oW)``, as returned by
    ``max_pool2d_with_indices``) index into the flattened spatial plane
    ``H*W`` of ``c`` (shape ``(N, C, H, W)``), per ``(N, C)``. Max pooling is
    piecewise linear, so every Taylor coefficient is selected at the same
    positions the primal's max chose.
    """
    N, C = c.shape[0], c.shape[1]
    selected = c.reshape(N, C, -1).gather(2, indices.reshape(N, C, -1))
    return selected.reshape(indices.shape)
